...
//	jpostdb.httpReq("get", url, null, group.plot_peptide, svg, renderDiv, param.width / 2, param.top);
	jpostdb.fetchReq("get", url, null, renderDiv, param.width, group.plot_peptide);
    },
    ...
    plot_peptide: function(json, renderDiv){
	var group = jpostdb.proteoform_browser;
	var param = group.param;
	var svg = renderDiv.select("#proteoform_browser_svg");
	...
	var setData = function(){
		...
		data.protein[i].top = param.top + h;
	...
	var plot = function(){
	  var prt = svg.selectAll("g.protein_group")
		...
		if(param.nameFrag) prt.selectAll("text.name").attr("display", "block");
	    }
	...
	var render = function(){
	  /*  var g = svg.append("g")
		...
		.on("mouseover", function(d){
		    ...
		    svg.select('#pop_up').attr("transform", "translate(" + x + "," + param.mouseY + ")");
		    svg.select('#pop_up_num').text(d.count + " pep");
	...
	for(var i = 1; i < data.protein.length; i++){
	    group.delUnit.mkButton(renderDiv, "protein_" + data.protein[i].up_id, data);
	}
	...
	var colorSwitch = function(){
		...
		var pep = prt.selectAll(".peptide_group")
		    .data(function(d){return d.peptide; });
		pep.selectAll("path.un_query_pep")
		    .attr("stroke", function(d){ return jpostdb.def_color2[(d.color - 1) % jpostdb.def_color2.length];});
		if(param.trypsinFrag) pep.selectAll("path.un_tryptic").attr("stroke", "#bbbbbb");
	    }
	};
	...
	var trypsinSwitch = function(){
		...
		pep.selectAll("path.un_tryptic")
		    .attr("stroke", function(d){ return jpostdb.def_color2[(d.color - 1) % jpostdb.def_color2.length];});
		if(!colorFrag) svg.selectAll("path.un_query_pep").attr("stroke", "#dddddd");
	    }else{
		param.trypsinFrag = 1;
		setData();
		plot();
    ...
    set_position: function(json, peptide){
	...
	for( var i = 0; i < seq_prt.length; i++){
	    if(up_list[seq_prt[i].up_id.value]) {
	...
	for(var i = 0; i < iso.length; i++){
	    var array = iso[i].jpo_iso.value.match(/([\w]{6})\-(\d+)$/);
	    hasIso[array[1]] = {};
	    hasIso[array[1]][array[2]] = 1;
	}

	for(var i = 0; i < pre_seq.length; i++){
	    var array = pre_seq[i].iso.value.match(/([\w]{6})\-(\d+)$/);
	    if(hasIso[array[1]] && up_list[array[1]]){
		if(hasIso[array[1]][array[2]]){
		    pre_seq[i].up_id = array[1] + "-" + array[2]; 
		    pre_seq[i].eval = up_list[array[1]];
		    seq.push(pre_seq[i]); 
		}
	    }else if(up_list[array[1]]){ 
		if(array[2] == 1){ 
...
console.log(b_max + ' ' + e_max);
	...
	var getExonPos = function(up, seq, exon){
		...
		if(exon[j].uniprot == up){
			...
			if(exon[j].isoform[k].sequence == seq){
			    var f = 0;
			    if(exon[j].isoform[k].location.match(/compement/)) f = 1;
	...
	return {base: base_length, sum_y: sum_y, protein: r1}
    },

    delUnit: {
	mkButton: function(renderDiv, gid, data){
	    var group = this;
		...
		.style("cursor", "pointer")
		.attr("display", "none")
	    	.on("click", function(){ group.remove(renderDiv, gid, data); });
	    g.append("rect")
	...
	rmF: {},
	remove: function(renderDiv, gid, data){
	    var param = jpostdb.proteoform_browser.param;
	    var svg = renderDiv.select("#proteoform_browser_svg");
	...
	mouseEveElement.on("mouseover", function(){ mouseOnElement = true;});
	mouseEveElement.on("mouseout", function(){ mouseOnElement = false;});
